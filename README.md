...
> OSS スキャナは常時実行するが、コンテナー／IaC のスキャンについては OSS ではあるものの重複を避けるためパイプライン本体では実行せず、Microsoft Security DevOps を有効化したときにのみ、同ステージ側で実行する。CodeQL（GitHub Advanced Security for Azure DevOps の機能）と Microsoft Security DevOps（Microsoft Defender for Cloud 連携）は有償ライセンスが前提のため、フラグで任意に有効化する。
...
| 項目 | 内容 | ライセンス |
|------|------|-----------|
| Azure DevOps 組織 / プロジェクト | パイプライン実行環境 | 無料枠 |
...
> **「無料枠」の範囲について**
> Azure DevOps の無料枠は組織単位で提供され、本教材のパイプラインは通常この範囲内で実行できる。ただし無料枠には次の上限がある。
>
> | 項目 | 無料枠の範囲（プライベートプロジェクト） |
> |------|----------------------------------------|
> | Basic ライセンス（ユーザー数） | 先頭 5 ユーザーまで無料 |
> | Microsoft-hosted 並列ジョブ（CI/CD） | **1 並列ジョブ**。1 ジョブあたり最長 **60 分**、合計 **月 1,800 分（30 時間）** まで |
> | self-hosted 並列ジョブ | 1 並列ジョブ（時間制限なし） |
> | Azure Repos | プライベート Git リポジトリ無制限 |
> | Azure Artifacts | 組織あたり **2 GiB** まで無料 |
>
> - 並列ジョブが 1 のため、複数ステージは**並列ではなく順次**実行される（パイプライン自体は完走する）。
> - 出典: [Billing overview（Free tier）](https://learn.microsoft.com/azure/devops/organizations/billing/overview#free-tier) / [Configure and pay for parallel jobs](https://learn.microsoft.com/azure/devops/pipelines/licensing/concurrent-jobs#how-much-do-parallel-jobs-cost) / [Microsoft-hosted agents](https://learn.microsoft.com/azure/devops/pipelines/agents/hosted#capabilities-and-limitations)（いずれも参照時点の最新を確認すること）
...
| Microsoft Security DevOps（Microsoft Defender for Cloud 連携） | 任意 | Azure RBAC（サブスクリプション）: **Owner** または **Security Admin**<br>Azure DevOps ロール（組織レベル）: **Project Collection Administrators** | 有償ライセンスが前提。連携手順は [`docs/setup/DEFENDER-INTEGRATION.md`](./docs/setup/DEFENDER-INTEGRATION.md) |
| Azure へのデプロイ（Azure Container Registry / Azure App Service）の準備 | 任意 | Azure RBAC（サブスクリプション／リソースグループ）: リソース作成に **Owner** / **Contributor**、RBAC 付与に **User Access Administrator** または **Owner**<br>Azure DevOps ロール（プロジェクトレベル）: **Endpoint Administrator**（または **Project Administrators**） | Web アプリケーションを実環境にデプロイして動作確認する場合のみ必要。手順は [`docs/setup/AZURE-SETUP.md`](./docs/setup/AZURE-SETUP.md) |
...
1. 本フォルダの内容を Azure DevOps の Git リポジトリにプッシュする。Azure DevOps の組織・プロジェクトがまだない場合は、先に [第1部（共通の事前準備）](./docs/setup/AZURE-SETUP.md#第1部-共通の事前準備必須) で作成する。
...
5. 実行完了後、各結果を確認する（詳細は [`docs/lab/LAB-GUIDE.md`](./docs/lab/LAB-GUIDE.md)）。
...
> - **使い終わり次第、削除する**（[後片付け](./docs/setup/AZURE-SETUP.md#後片付け)）。最も安全。
> - 残す場合は、**Azure App Service のアクセス制限で自分の IP のみ許可する**（手順: [アクセスを自分の IP のみに制限する](./docs/setup/AZURE-SETUP.md#8-アクセスを自分の-ip-のみに制限する強く推奨)）。

> **Azure Container Registry や Azure App Service などの Azure リソースはパイプラインでは自動作成されない。** リソースの作成、サービス接続・RBAC の設定、そして作成した値をパイプライン変数のどこに反映するかの詳細な GUI 操作手順は **[`docs/setup/AZURE-SETUP.md`](./docs/setup/AZURE-SETUP.md)** を参照すること。

### 環境準備概要（詳細は事前準備ガイド）

1. [`docs/setup/AZURE-SETUP.md`](./docs/setup/AZURE-SETUP.md) に従って Azure リソースを作成する。
...
対応する拡張・ライセンスが未導入の場合は該当ステージはプレースホルダのまま終了する（詳細は  [`docs/setup/DEFENDER-INTEGRATION.md`](./docs/setup/DEFENDER-INTEGRATION.md) を参照）。
...
修正方法は [`docs/lab/SOLUTION.md`](./docs/lab/SOLUTION.md) を参照すること。
