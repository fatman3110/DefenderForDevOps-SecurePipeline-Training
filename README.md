...
| 作業 | 必要なロール | 補足 |
|------|------------|------|
| SARIF SAST Scans Tab 拡張のインストール | Azure DevOps ロール（組織レベル）: **Project Collection Administrators**（または組織 Owner） | Microsoft Entra ID のロールでも Azure RBAC のロールでもない。権限がない一般ユーザーは拡張を「リクエスト」し、上記管理者が承認・インストールする |
| パイプラインの作成・実行 | Azure DevOps ロール（プロジェクトレベル）: **Contributors** | 通常のプロジェクトメンバーで可 |
| CodeQL（GitHub Advanced Security for Azure DevOps）の有効化 | Azure DevOps ロール（プロジェクトレベル）: **Project Administrators** | 課金の有効化には組織 Owner が必要 |
| Microsoft Security DevOps（Microsoft Defender for Cloud 連携） | Azure RBAC（サブスクリプション）: **Owner** または **Security Admin**<br>Azure DevOps ロール（組織レベル）: **Project Collection Administrators** | 連携手順は [`docs/DEFENDER-INTEGRATION.md`](./docs/DEFENDER-INTEGRATION.md) |
| Azure へのデプロイ（Azure Container Registry / Azure App Service）の準備 | Azure RBAC（サブスクリプション／リソースグループ）: リソース作成に **Owner** / **Contributor**、RBAC 付与に **User Access Administrator** または **Owner**<br>Azure DevOps ロール（プロジェクトレベル）: **Endpoint Administrator**（または **Project Administrators**） | 手順は [`docs/AZURE-SETUP.md`](./docs/AZURE-SETUP.md) |
...
1. 本フォルダの内容を Azure DevOps の Git リポジトリにプッシュします。Azure DevOps の組織・プロジェクトがまだない場合は、先に [`docs/AZURE-SETUP.md`](./docs/AZURE-SETUP.md) の**第1部（共通の事前準備）**で作成します。
...
4. パイプラインを実行します。次のいずれかの方法で実行できます。

   **4-1. 任意のタイミングで手動実行する（コード変更なし）**

   1. **Pipelines** から対象のパイプラインを開きます。
   2. 右上の **Run pipeline**（パイプラインの実行）をクリックします。
   3. 実行するブランチ（例: `master`）を選び、**Run** をクリックします。
   4. すぐにパイプラインが起動し、ビルド・テスト・各スキャンが実行されます。

   > 手順 3 で初めてパイプラインを作成した直後は、自動的に 1 回目の実行が開始されます。

   **4-2. 新しくコードをコミットして実行する（CI トリガー）**

   `azure-pipelines.yml` の `trigger` に指定されたブランチ（`main` / `master`）へコミットを push すると、パイプラインが自動で起動します。脆弱性を修正してから再実行し、スキャン結果が改善することを確認する流れに使います。
   ...
   > `*.md` や `docs/` 配下だけの変更は `trigger` の `paths` 除外により起動しません（ドキュメント編集ではパイプラインが回りません）。
...
パイプラインの `Deploy` ステージは、ビルドしたコンテナイメージを **Azure Container Registry** にプッシュし、**Azure App Service**（PaaS ランタイム実行環境） にデプロイします。デプロイ後は Azure App Service の URL `https://<webAppName>.azurewebsites.net` でアプリにアクセスでき、脆弱性の手動確認（XSS・アクセスコントロール・Cookie など）が当該 URL 上で行えます。
本手順を行わない場合でも、CI/CD パイプラインの実行と脆弱性の確認が可能です。

> **Azure Container Registry や Azure App Service などの Azure リソースはパイプラインでは自動作成されません。** リソースの作成、サービス接続・RBAC の設定、そして作成した値をパイプライン変数のどこに反映するかの詳細な GUI 操作手順は **[`docs/AZURE-SETUP.md`](./docs/AZURE-SETUP.md)** を参照してください。

### 環境準備概要（詳細は事前準備ガイド）

1. [`docs/AZURE-SETUP.md`](./docs/AZURE-SETUP.md) に従って Azure リソース作成します。
2. リソース作成時に控えた値を [`azure-pipelines.yml`](./azure-pipelines.yml) の `variables` に反映したうえで、環境へのデプロイを有効化します。
...
3. パイプラインを実行します。`Deploy` ステージが Azure Container Registry へのプッシュと Azure App Service へのデプロイを行います。
...
対応する拡張・ライセンスが未導入の場合は該当ステージはプレースホルダのまま終了します（詳細は  [`docs/DEFENDER-INTEGRATION.md`](./docs/DEFENDER-INTEGRATION.md) を参照）。
...
- アプリケーションは意図的に脆弱性を含みます。**隔離された検証環境またはローカルでのみ実行してください。**
- 各スキャンステップは教材向けに `continueOnError: true` としており、脆弱性を検出してもパイプラインは継続します。実運用では閾値に応じたエラーハンドリングを設定してください。
